 ...
 */
export const message = (that, type, message, linkUrl, center, iconClass, dangerouslyUseHTMLString) => {
  const params = { that, type, message, linkUrl, center, iconClass, dangerouslyUseHTMLString }
  params.showClose = true
  if (linkUrl) { // 如果超链接存在
    const h = that.$createElement
    params.showClose = false
    params.message = h('p', { class: 'message-content' }, [
      h('span', { class: 'message-info' }, message),
      h('a', {
        attrs: {
          href: `#${linkUrl.url}`
        },
 ...
 */
export const notification = (that, title, message, type, dangerouslyUseHTMLString, callBack, iconClass) => {
  const params = { that, title, message, type, iconClass }
 ...
 */
export const confirm = (that, title, content, confirmCallback, isConfirm, type) => {
 ...
 * @param message 提示信息
 */
export const autoCloseMessage = (that, type, message) => {
  that.$message({
    type, message,
